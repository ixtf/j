...
/**
 * // 环境检测 val isCI = System.getenv("CI") == "true" val versionTag = System.getenv("CI_COMMIT_TAG")
 * if (versionTag?.isNotBlank() == true) version = versionTag.removePrefix("v") val scmInfo by lazy
 * { if (isCI) { mapOf( "branch" to System.getenv("CI_COMMIT_REF_NAME"), "commitId" to
 * System.getenv("CI_COMMIT_SHA"), "commitTime" to System.getenv("CI_COMMIT_TIMESTAMP"), ) } else {
 ...
 *   }
 *
 * tasks.withType<Jar> { manifest { attributes( "Implementation-Title" to project.name,
 * "Implementation-Version" to project.version, "Created-By" to
 * "${System.getProperty("java.version")} (${System.getProperty("java.vendor")})", "Build-Timestamp"
 * to Instant.now().toString(), "SCM-Branch" to "${scmInfo["branch"]}", "SCM-Commit-id" to
 * "${scmInfo["commitId"]}", "SCM-Commit-id-abbrev" to "${scmInfo["commitId"]?.take(8)}",
 * "SCM-Commit-Time" to "${scmInfo["commitTime"]}", ) } }
 *
 * publishing { repositories { maven { name = "GitHubPackages" url =
 * uri("https://maven.pkg.github.com/ixtf/gpr") credentials { username =
 * project.findProperty("gpr.user") as String? ?: System.getenv("GPR_USER") password =
 * project.findProperty("gpr.key") as String? ?: System.getenv("GPR_KEY") } } }
 *
 * publications { create<MavenPublication>("mavenJava") { from(components["java"]) versionMapping {
 * usage("java-api") { fromResolutionOf("runtimeClasspath") } usage("java-runtime") {
